...
def neural_network(feat):
    ...
    X=feat.drop(['Link', 'C_Neighbour', 'P_Attachment',
       'Jaccard_Index', 'R_Allocation', 'Salton_Cosine',
       'Sorensen_Index', 'Promoted_Index', 'Depressed_Index', 'LHN_Index'],axis=1)
    y=feat['Link']
...
def knearest_neighbour(feat_temp):
    ...
    X=feat.drop(['C_Neighbour', 'P_Attachment', 'Jaccard_Index', 'R_Allocation', 'Salton_Cosine',
       'Sorensen_Index', 'Promoted_Index', 'Depressed_Index', 'LHN_Index'],axis=1)
    y=feat_temp['Link']
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=42)
