...
def jaccard_coefficient(G,feat,ledge):
    feat['Jaccard_Index']=0.0
    for i in feat.index.values:
        if(G.has_node(feat['Node_1'][i]) and G.has_node(feat['Node_2'][i])):
            values=nx.jaccard_coefficient(G,[(feat['Node_1'][i],feat['Node_2'][i])])
            for v in values:
                feat['Jaccard_Index'][i]=v[2]
...
jaccard_coefficient(G_f, f_feat, f_ledge)
...
jaccard_coefficient(G_ff, ff_feat, ff_ledge)
...
jaccard_coefficient(G_fs, fs_feat, fs_ledge)
...
jaccard_coefficient(G_h, h_feat, h_ledge)
...
jaccard_coefficient(G_v, v_feat, v_ledge)
