...
def preferential_attachment(G,feat,ledge):
    feat['P_Attachment']=0
    for i in feat.index.values:
        if(G.has_node(feat['Node_1'][i]) and G.has_node(feat['Node_2'][i])):
            values=nx.preferential_attachment(G,[(feat['Node_1'][i],feat['Node_2'][i])])
            for v in values:
                feat['P_Attachment'][i]=v[2]
...
preferential_attachment(G_f, f_feat, f_ledge)
...
preferential_attachment(G_ff, ff_feat, ff_ledge)
...
preferential_attachment(G_fs, fs_feat, fs_ledge)
...
preferential_attachment(G_h, h_feat, h_ledge)
...
preferential_attachment(G_v, v_feat, v_ledge)
