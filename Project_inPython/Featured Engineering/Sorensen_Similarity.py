...
def sorensen(G,feat,ledge):
    feat['Sorensen_Index']=0.0
    for i in feat.index.values:
        if(G.has_node(feat['Node_1'][i]) and G.has_node(feat['Node_2'][i])):
            ...
            feat['Sorensen_Index'][i]=feat['C_Neighbour'][i]/(node1+node2)
...
sorensen(G_ff, ff_feat, ff_ledge)
...
sorensen(G_fs, fs_feat, fs_ledge)
