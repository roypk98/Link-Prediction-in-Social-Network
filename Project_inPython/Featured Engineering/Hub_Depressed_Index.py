...
def depressed_index(G,feat,ledge):
    feat['Depressed_Index']=0.0
    for i in feat.index.values:
        if(G.has_node(feat['Node_1'][i]) and G.has_node(feat['Node_2'][i])):
            ...
            feat['Depressed_Index'][i]=feat['C_Neighbour'][i]/max(node1,node2)
...
depressed_index(G_f, f_feat, f_ledge)
...
depressed_index(G_ff, ff_feat, ff_ledge)
...
depressed_index(G_fs, fs_feat, fs_ledge)
...
depressed_index(G_h, h_feat, h_ledge)
...
depressed_index(G_v, v_feat, v_ledge)
