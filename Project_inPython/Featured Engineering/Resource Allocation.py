...
def resource_allocation(G,feat,ledge):
    ...
    for i in feat.index.values:
        if(G.has_node(feat['Node_1'][i]) and G.has_node(feat['Node_2'][i])):
            values=nx.resource_allocation_index(G,[(feat['Node_1'][i],feat['Node_2'][i])])
            for v in values:
...
resource_allocation(G_f, f_feat, f_ledge)
...
resource_allocation(G_ff, ff_feat, ff_ledge)
...
resource_allocation(G_fs, fs_feat, fs_ledge)
...
resource_allocation(G_h, h_feat, h_ledge)
...
resource_allocation(G_v, v_feat, v_ledge)
